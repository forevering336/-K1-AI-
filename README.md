...
本项目使用 Roboflow Universe 公开焊接缺陷数据集，类别为 Crack、Porosity、Spatters、Welding line。完整训练/验证/测试图片不属于运行必需文件，已移入 `非提交材料_数据集训练缓存/`。如需重新训练或复现实验，可将以下目录移回项目根目录；`data.yaml` 和 `data_weld_local.yaml` 默认使用 `path: .`：
...
当前版本已经整理为最终交付结构：主程序、Web 看板、声光报警、模型、评估日志、图表和报告材料均已归位。后续演示阶段建议补充 K1 实机运行截图和 3 分钟以内演示视频。
